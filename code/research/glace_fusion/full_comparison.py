...
def main():
    ...
    if not quality.get('ready_for_test'):
        raise SystemExit('Head has not passed training-data localization checks')
    ...
    for date in TEST_DATES:
        if not (args.camera_root / date / '.complete').exists():
            raise SystemExit('Incomplete test imagery: ' + date)
    ...
    loader = DataLoader(IndexedSubset(dataset, list(range(len(records), total))), batch_size=args.batch_size,
    ...
    with records_path.open('a', buffering=1) as output, torch.inference_mode():
        for batch in loader:
            ...
            for position, index in enumerate(batch['indices']):
                ...
                if delta is not None and abs(delta) <= .05:
                    row = camera[date][j]
                ...
                record = {'index': int(index), 'sequence': date, 'scan_timestamp': ts, 'sync_delta_s': delta,
                          'status': status, 'errors': {name: errors(T, gt) for name, T in zip(methods, poses)}}
