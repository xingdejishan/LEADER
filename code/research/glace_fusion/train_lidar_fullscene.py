...
def parse_args():
    ...
    parser.add_argument('--aux-weight', type=float, default=1.0,
                        help='overall auxiliary loss weight')
    parser.add_argument('--aux-mode', default='decomposed',
                        choices=['decomposed', 'log_depth', 'l1'],
                        help='decomposed (default): pixel bearing + log-depth; legacy modes kept')
    ...
    parser.add_argument('--no-reliability-head', action='store_true',
                        help='disable the per-cell reliability head (improvement B)')
    parser.add_argument('--reliability-lr', type=float, default=1e-3)
    parser.add_argument('--depth-ratio-tol', type=float, default=1.25)
    parser.add_argument('--scan-window-s', type=float, default=0.4,
                        help='aggregate velodyne_sync scans within +-window of the image '
                             'timestamp (0 = exact timestamp only)')
...
def prepare_vendor(args, config_stage1):
    ...
    patch_lidar_supervision(vendor, weight=args.aux_weight, relative_storage=True,
                            aux_mode=args.aux_mode, bearing_weight=args.bearing_weight,
                            depth_weight=args.depth_weight,
                            bearing_beta_px=args.bearing_beta_px,
                            reliability_head=not args.no_reliability_head,
                            reliability_lr=args.reliability_lr,
                            depth_ratio_tol=args.depth_ratio_tol)
    return vendor
...
def main():
    ...
    try:
        ...
        written, missing, scan_counts = build_targets(
            ...
            scan_window_s=args.scan_window_s,
            progress_path=args.run_root / 'lidar_targets.progress.json')
        mean_scans = float(np.mean([scan_counts[s] for s in written])) if written else 0.0
        ...
        config = dict(train_args=train_args, aux_weight=args.aux_weight,
                      aux_mode=args.aux_mode, bearing_weight=args.bearing_weight,
                      depth_weight=args.depth_weight, bearing_beta_px=args.bearing_beta_px,
                      reliability_head=not args.no_reliability_head,
                      reliability_lr=args.reliability_lr,
                      depth_ratio_tol=args.depth_ratio_tol,
                      scan_window_s=args.scan_window_s,
                      mean_scans_per_image=mean_scans,
