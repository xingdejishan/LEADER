...
def get_sector_angles(sections, r):
    angles = torch.arange(0, 2 * math.pi, 2 * math.pi / sections) + math.pi / sections
    q_rot = torch.zeros((sections, 3, 3), device=r.device)
    cos_angles = torch.cos(angles)
    sin_angles = torch.sin(angles)
    q_rot[:, 0, 0] = cos_angles
    q_rot[:, 0, 1] = -sin_angles
    q_rot[:, 1, 0] = sin_angles
    q_rot[:, 1, 1] = cos_angles
    q_rot[:, 2, 2] = 1
    r_sections = torch.matmul(q_rot.unsqueeze(0), r.unsqueeze(1).unsqueeze(3)).squeeze()
    return r_sections
...
def filter_overflow_mulran(gt_filename,  ts_raw):
    ground_truth = np.loadtxt(gt_filename, delimiter=',')[:, 0]
    min_pose_timestamps = min(ground_truth)
    max_pose_timestamps = max(ground_truth)
    ts_filted = [t for t in ts_raw if min_pose_timestamps < t < max_pose_timestamps]
    abandon_num = len(ts_raw) - len(ts_filted)
...
def interpolate_pose_mulran(gt_filename, ts_raw):
    return interpolate_pose_nclt(gt_filename, ts_raw)
...
def cartesian_to_polar_expansion(points, horizontal=256.0):
    angles = np.arctan2(points[:, 1], points[:, 0]).clip(-np.pi, np.pi-1e-6)
    ranges = np.linalg.norm(points[:, :2], axis=1, keepdims=True)
    highs = points[:, 2:3]

    x = angles[..., np.newaxis] * horizontal / (np.pi * 2)
    return np.concatenate([x, ranges, highs], axis=1)

def polar_expansion_to_cartesian(points, horizontal=256.0):
    angles = points[:, 0:1] * np.pi * 2 / horizontal
    ranges = points[:, 1:2]
    highs = points[:, 2:3]

    x = ranges * torch.cos(angles)
    y = ranges * torch.sin(angles)
    return torch.cat([x, y, highs], dim=1)

# def cartesian_to_polar_expansion(points, horizontal=256.0):
#     angles = np.arctan2(points[:, 1], points[:, 0]).clip(-np.pi, np.pi-1e-6)
#     ranges = np.linalg.norm(points[:, :3], axis=1, keepdims=True)
#     pitch = np.arctan2(points[:, 2], np.linalg.norm(points[:, :2], axis=1))

#     x = angles[..., np.newaxis] * horizontal / (np.pi * 2)
#     z = pitch[..., np.newaxis] * 0.2 * 180 / (np.pi)
#     return np.concatenate([x, ranges, z], axis=1)

# def polar_expansion_to_cartesian(points, horizontal=256.0):
#     angles = points[:, 0:1] * np.pi * 2 / horizontal
#     ranges = points[:, 1:2]
#     pitch = points[:, 2:3] * np.pi / (180 * 0.2)

#     x = ranges * torch.cos(pitch) * torch.cos(angles)
#     y = ranges * torch.cos(pitch) * torch.sin(angles)
#     highs = ranges * torch.sin(pitch)
#     return torch.cat([x, y, highs], dim=1)
