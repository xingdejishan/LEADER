...
def main():
    ...
    parser.add_argument('--variant', choices=('box', 'surface'), required=True)
    ...
    missing, extra = model.magic_fusion.load_state_dict(init['fusion'], strict=False)
    expected = {f'attention.{i}.relative_{kind}.weight'
                for i in range(3) for kind in ('key', 'value')} if args.variant == 'surface' else set()
    ...
    settings.update(protocol='surface_token_trial_v1', magic=True, fusion_variant=args.variant,
                    ...
                    source_sha256={name: digest(Path(name)) for name in (
                        ...
                        'models/surface_token_fusion.py', 'tools/train_surface_trial.py',
                        'tools/train_local905.py', 'tools/eval_local905_online.py',
                        'data/local905_query.py', 'experiments/surface_token/PLAN.md')})
