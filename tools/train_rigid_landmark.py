...
def evaluate(checkpoint, directory, subset, args):
    run(['tools.eval_rigid_landmark', '--cache', args.cache, '--pose_cache', args.pose_cache, '--checkpoint', checkpoint,
...
def main():
    ...
    source_names = ('models/landmark_memory.py', 'models/rigid_landmark.py', 'tools/build_rigid_cache.py',
                    'tools/rigid_landmark_data.py', 'tools/eval_rigid_landmark.py', 'tools/train_rigid_landmark.py',
                    'tests/test_rigid_landmark.py', 'tools/eval_local905_gt.py',
                    'models/sc2pcr.py', 'utils/full_pool_robust_v1.py',
                    'experiments/rigid_landmark/PLAN.md', 'experiments/rigid_landmark/audit.json')
    protocol = dict(name='rigid_landmark_v1', source_commit=args.source_commit,
    ...
    model = RigidLandmarkFusion().cuda()
    ...
    training = frames(args.cache, args.pose_cache, 'train')
    ...
    with (args.out / 'history.jsonl').open('w') as stream:
        while True:
            ...
            for offset in range(0, count, 8):
                ...
                pose = predict(model, reference, batch)
