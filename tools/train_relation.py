...
def setup(args):
    ...
    if digest(args.assets/'split_masked.json') != base['settings']['split_sha256']:
        raise ValueError('Training split differs from pretrained baseline protocol')
    model = LEADER(in_channels=3, out_channels=4, magic=True, fusion_variant='relation').cuda()
...
def main():
    ...
    settings = dict(base['settings'], variant='relation_transport', magic=True, fusion_variant='relation',
    ...
    sources = ('models/model_mink.py', 'models/interaction_backbone.py', 'models/relation_interaction.py', 'models/relation_graph.py', 'models/surface_token_fusion.py',
        'models/magic_fusion.py', 'tools/train_relation.py', 'tools/train_local905.py',
        'tools/eval_local905_online.py', 'tools/eval_local905_gt.py', 'tools/verify_relation.py', 'tools/verify_interaction.py',
        'data/local905_mink.py', 'data/local905_query.py', 'data/magic_data.py',
        'run_mink.py', 'models/sc2pcr.py', 'utils/full_pool_robust_v1.py',
        'tools/train_magic_revision.py', 'tools/train_gravity_fusion.py', 'tools/compare_local905.py',
        'tools/run_surface_trial.py', 'tests/test_relation_graph.py',
        'experiments/relation_transport/PLAN.md', 'experiments/relation_transport/audit.json',
        'experiments/relation_transport/ref_search.json')
