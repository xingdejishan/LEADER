...
#!/usr/bin/env python
"""Configuration script for setting up tab completion for scrstudio in bash and zsh."""
...
HEADER_LINE = "# Source scrstudio autocompletions."


def _get_all_entry_points() -> List[str]:
    # TODO: we should filter out entrypoints that are not tyro CLIs.
    entry_points = importlib_metadata.distribution("scrstudio").entry_points
...
def _get_conda_path() -> Optional[pathlib.Path]:
    ...
    the scrstudio package is installed in one.
    """
    conda_path = None
    if "CONDA_PREFIX" in os.environ:
        # Conda is active, we will check if the Nerfstudio is installed in the conda env.
        distribution = importlib_metadata.distribution("scrstudio")
        if str(distribution.locate_file("scrstudio")).startswith(os.environ["CONDA_PREFIX"]):
            conda_path = pathlib.Path(os.environ["CONDA_PREFIX"])
    return conda_path
