...
from scrstudio.configs import base_config as cfg
from scrstudio.data.datamanagers.base_datamanager import DataManager, DataManagerConfig
from scrstudio.data.datamanagers.glbuffer_datamanager import GLBufferDataManager, GLBufferDataManagerConfig
...
class VanillaPipelineConfig(cfg.InstantiateConfig):
    ...
    datamanager: DataManagerConfig = field(default_factory=lambda: GLBufferDataManagerConfig())
    """specifies the datamanager config"""
    model: ModelConfig =field(default_factory=lambda: ModelConfig())
    """specifies the model config"""
    num_pose_workers: int = 3
    """number of workers for pose estimation"""
    acc_thr: str = "aachen"
...
class VanillaPipeline(Pipeline):
    ...
    def __init__(
        ...
        self.pose_pool = Pool(config.num_pose_workers)
        self.datamanager: GLBufferDataManager = config.datamanager.setup(
            device=device, test_mode=test_mode, world_size=world_size, local_rank=local_rank, mixed_precision=mixed_precision
        )
